export class FxEngine {
  ...
  private _audioEaster;
  private _audioBackground;
  ...
  private _initializeShai(context) {
    this._shaiVoices = this._shaiVoices.map( (src) => {
  ...
  private _initializeFx(context) {
    this._fx = this._fx.map( (src) => {
  ...
  private _initializeEaster(args) {
    let {source, audio} = this._initializeAudio(args);
    this._audioEaster = audio;
    return source;
  }

  private _initializeBackground(args) {
    let {source, audio} = this._initializeAudio(args);
    this._audioBackground = audio;
    return source;
  }

  private _initializeAudio([context, gain=1, src, controls=0, autoplay=0, loop=0]) {
